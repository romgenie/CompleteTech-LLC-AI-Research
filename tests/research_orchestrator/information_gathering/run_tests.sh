#!/bin/bash
# Script to run information gathering tests with different options
...
# Help text
show_help() {
    echo -e "${BLUE}Information Gathering Test Runner${NC}"
    echo "Usage: ./run_tests.sh [options]"
    echo ""
    echo "Options:"
    echo "  -t, --test-type TYPE    Run specific test types: unit, integration, e2e, property, benchmark, edge, all (default: all)"
    echo "  -m, --markers MARKERS   Specify pytest markers (e.g. 'search or source')"
    ...
    echo "  ./run_tests.sh -m search          # Run only search-related tests"
