...
const KnowledgeGraphPage: React.FC = () => {
  ...
  // Benchmark mode
  const [benchmarkMode, setBenchmarkMode] = useState<boolean>(false);
  ...
  const [loadedNodeCount, setLoadedNodeCount] = useState<number>(0);
  ...
  useEffect(() => {
    ...
    if (graphData && graphData.nodes.length > 0) {
      if (graphData.nodes.length > visualizationSettings.filterThreshold!) {
  ...
  const renderGraph = useSvgD3<SVGSVGElement>((svg) => {
    ...
    function drag(simulation: d3.Simulation<Entity, undefined>) {
      ...
      return d3.drag<SVGCircleElement, Entity>()
  ...
  const fetchEntityDetails = async (entityId: string) => {
    ...
    try {
      ...
      if (entity) {
        ...
        setEntityDetails(mockEntityDetails as Entity);
      } else {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      setSearchResults(searchResults as Entity[]);
      setSelectedEntity(null);
      setEntityDetails(null);
      setRelatedEntities([]);
      setGraphData(null);
                        ...
                        secondary={
                          <Box display="flex" alignItems="center" mt={0.5}>
                            ...
                            <Typography variant="caption" color="text.secondary">
                              Relevance: {(entity as any).relevance || 'N/A'}
                            </Typography>
                          </Box>
                        } 
                      />
                  ...
                  loading ? (
                    ...
                    <Box>
                      ...
                      <Box sx={{ maxHeight: '18vh', overflowY: 'auto', pr: 1 }}>
                        <Grid container spacing={2}>
                          {entityDetails.properties && Object.entries(entityDetails.properties as Record<string, any>).map(([key, value]) => (
                        ...
                        {(entityDetails as any).description && (
                          <>
                            <Divider sx={{ my: 2 }} />
